...
<head>
    ...
    <style>
        ...
        /* Stats Grid Fix */
        .stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin: 25px 0; }
        .stat-item { display: flex; align-items: center; gap: 14px; padding: 14px 16px; background: #f9f9f9; border-radius: 10px; }
        .stat-icon { font-size: 22px; color: #2c7be5; min-width: 28px; text-align: center; }
        ...
        .pdf-viewer-container { position: relative; width: 100%; height: 500px; background: #f1f5f9; border-radius: 8px; overflow: hidden; margin-bottom: 20px; border: 1px solid #e5e7eb; }
        .pdf-iframe { width: 100%; height: 100%; border: none; }
        
        .blueprint-box { margin-top: 24px; padding: 16px; border: 1px solid #e2e8f0; border-radius: 12px; background: #f8fafc; }
        .blueprint-box img { width: 100%; border-radius: 10px; display: block; }
        
        .tab-btn { padding: 8px 16px; border-radius: 8px; border: 1px solid #e5e7eb; background: #f3f4f6; font-weight: 700; cursor: pointer; }
        .tab-btn.active { background: #fff; color: var(--primary); border-bottom: 2px solid var(--primary); }
        ...
        @media (max-width: 991px) { .layout-grid { grid-template-columns: 1fr; } .sticky-col { position: static; } .pdf-viewer-container { height: 400px; } }
    </style>
</head>
<body>
                    ...
                    <div class="card">
                        ...
                        <div class="stats">
                            <div class="stat-item">
                                ...
                                <div><span class="stat-label">Total Area</span><span class="stat-value"><?php echo htmlspecialchars($property['size']); ?> SQFT</span></div>
                            </div>
                            <div class="stat-item">
                                <span class="stat-icon"><i class="fa-solid fa-up-right-and-down-left-from-center"></i></span>
                                <div><span class="stat-label">Dimensions (w*N*E*S)</span><span class="stat-value"><?php echo htmlspecialchars($property['dimensions']); ?></span></div>
                            </div>
                            <div class="stat-item">
                                <span class="stat-icon"><i class="fa-solid fa-compass"></i></span>
                                <div><span class="stat-label">Facing</span><span class="stat-value"><?php echo htmlspecialchars($property['facing']); ?></span></div>
                            </div>
                            <div class="stat-item">
                                <span class="stat-icon"><i class="fa-solid fa-house"></i></span>
                                <div><span class="stat-label">Type</span><span class="stat-value"><?php echo htmlspecialchars($property['type']); ?></span></div>
                            </div>
                        </div>

                        <h3 style="color: var(--dark); margin: 30px 0 15px 0;">Property Description</h3>
                        <p style="line-height: 1.8; color: #555; font-size: 15px;"><?php echo nl2br(htmlspecialchars($property['description'])); ?></p>
                        ...
                        <div style="margin-top:30px;">
                            <?php if ($blueprintImg || !empty($documents)): ?>
                                <div style="display:flex; gap:12px; margin-bottom:15px;">
                                    <?php if ($blueprintImg): ?>
                                        <button type="button" class="tab-btn active" data-tab="tab-blueprint">Blueprint</button>
                                    <?php endif; ?>
                                    <?php if (!empty($documents)): ?>
                                        <button type="button" class="tab-btn <?= !$blueprintImg ? 'active' : '' ?>" data-tab="tab-documents">Documents</button>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="blueprint-box">
                                    <?php if ($blueprintImg): ?>
                                        <div id="tab-blueprint" class="tab-content active">
                                            <img src="<?= htmlspecialchars($blueprintImg) ?>" oncontextmenu="return false;">
                                        </div>
                                    <?php endif; ?>

                                    <div id="tab-documents" class="tab-content <?= !$blueprintImg ? 'active' : '' ?>">
                                        <?php if (!empty($documents)): ?>
                                            <?php foreach ($documents as $doc): ?>
                                                <p style="font-weight:700; margin:15px 0 8px;"><?= htmlspecialchars($doc['title']) ?></p>
                                                <div class="pdf-viewer-container" oncontextmenu="return false;">
                                                    <iframe src="<?= htmlspecialchars($doc['path']) ?>#toolbar=0&navpanes=0" class="pdf-iframe"></iframe>
                                                </div>
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <p style="text-align:center; padding:20px; color:#666;">No documents available.</p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            <?php endif; ?>
                ...
                <div class="right-col">
                    <div class="card sticky-col">
                        ...
                        <form class="inquiry-form" action="submit-enquiry.php" method="POST">
                            ...
                            <textarea name="message" rows="5">I am interested in <?php echo htmlspecialchars($property['name']); ?>.</textarea>
